...
switch = 0
SS = []
with open(target_path+"/second_structure")as f:
    lines = f.readlines()
    for line in lines:
...
Fscore = []
with open(target_path+"/ResFsocre")as t:
    for line in t.readlines():
        Fscore.append(line.split()[0:2])
...
while success == 0:
    ...
    for i in range(len(all_vec_ss) - 1):
        for j in range(int(all_vec_ss[i]), int(all_vec_ss[i + 1])):
            if float(Fscore[j][1]) > float(max_Fscore):
                max_Fscore = Fscore[j][1]
        if float(max_Fscore) < int(cut_off):
    ...
    if float(inter_len) >= float(0.75 * len(SS)) and small == 0:
        cut_off += 10
    if float(inter_len) < float(0.75 * len(SS)):
        success = 1

vec_inter_res = []
with open(target_path+"/inter_index", 'w') as outfile:
    for i in range(len(all_vec_ss_01)):
...
with open(target_path+"/Template_for_I")as f:
    for num, line in enumerate(f):
        if num == 0:
            temp = line.split()[0:1]
            break
    with open(target_path+"/output/"+temp[0]+".pdb")as t, open(target_path+"/intermediate.pdb", 'w') as outfile:
