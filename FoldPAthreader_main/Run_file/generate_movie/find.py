...
out_index = sys.argv[3]

int_TMscore_cut = 0
with open(path_tran+'/pdb_inter_index') as f:
    lines = f.readlines()
    int_TMscore_cut = lines[0:1]
...
print(float(TMscore[0].split()[2]))

if (float(pdb_index) > int(int_TMscore_cut[0]) and float(TMscore[0].split()[2]) != 1.0) or (float(pdb_index) <= int(int_TMscore_cut[0]) and float(TMscore[0].split()[2]) <= 0.99):
    print('--------')
    line = []
    ...
    with open(path_tran+'/output_pdb'+out_index+'/model_'+pdb_index+'.pdb', 'r') as f, open(path_tran+'/tran_out_pdb/model_'+pdb_index+'.pdb', 'w') as outfile:
        for line in f.readlines():
            if (line[0:6] != "ATOM  "):
                continue
